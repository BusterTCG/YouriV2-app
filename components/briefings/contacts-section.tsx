...
import { Loader2, Plus, Trash2, UserPlus, UserSearch } from "lucide-react";
...
export function ContactsSection({ briefingId, rows }: Props) {
  ...
  const [addingMode, setAddingMode] = useState<null | "linked" | "inline">(
    null,
  );
  ...
  function submit() {
    ...
    startTransition(async () => {
      ...
      const res = await addBriefingContact({
        ...
        // ContactSnapshot n'expose pas phone/email — l'user peut les ajouter
        // côté ContactKN puis re-rafraîchir, ou utiliser le mode inline pour
        // saisir directement. Pour Lot B3 on garde simple : pas de fetch
        // approfondi du Contact KN (1 round-trip de plus pas justifié pour
        // ce MVP).
        phone: null,
        email: null,
        role,
